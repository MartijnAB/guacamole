...
import org.hammerlab.guacamole.filters.{ SomaticAlternateReadDepthFilter, SomaticReadDepthFilter, PileupFilter }
...
object BCaller {
  ...
  object Caller extends SparkCommand[Arguments] {
    ...
    override def run(args: Arguments, sc: SparkContext): Unit = {
      ...
      val interval = args.interval / 100.0
      ...
      Common.progress("Computed %,d genotypes after basic filtering".format(potentialGenotypes.count))
    ...
    def findPotentialVariantAtLocus(tumorPileup: Pileup,
                                    ...
                                    interval: Double = 0.95): Seq[CalledSomaticAllele] = {
      ...
      if (tumorLowerBound > normalUpperBound) {
        for {
